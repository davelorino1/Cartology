  ...
  END IF;
    ...
    CREATE OR REPLACE TABLE gcp-wow-cart-data-dev-d4d7.davide.baseline_statistics_with_campaign AS
        WITH campaign_start_date AS (
            SELECT DISTINCT campaign_start_date 
            FROM gcp-wow-cart-data-dev-d4d7.davide.instore_screens_campaigns_june_2023_onwards_2
            WHERE booking_and_asset_number = current_campaign_global_var
        ),
        ...
        baseline_weekly_sales AS (
            ...
            WHERE ass_campaign_period.TXNStartDate BETWEEN DATE_SUB(campaign_start_date.campaign_start_date, INTERVAL 12 WEEK) AND DATE_SUB(campaign_start_date.campaign_start_date, INTERVAL 1 WEEK)
    ...
    --CREATE OR REPLACE TABLE gcp-wow-cart-data-dev-d4d7.davide.instore_screens_sales_pre_vs_during_period_plus_baseline_2 AS 
    SELECT 
